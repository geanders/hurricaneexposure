...
You can also use the `map_tracks` function to add a storm track to an existing U.S. map, as long as the map is a `ggplot` object. You do this by specifying the map you'd like to use as a base in the `plot_object` argument of `map_tracks`. For example, to add the storm track to the plot of distance exposure for Sandy or rain exposure for Floyd, you can run:
...
```{r fig.width = 7, fig.height = 4}
floyd_map <- map_rain_exposure(storm = "Floyd-1999", rain_limit = 125, dist_limit = 500)
